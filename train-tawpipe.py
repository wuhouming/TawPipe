...
if __name__ == "__main__":
    ...
    if enable_prof:
        prof.stop()
        prof.export_chrome_trace(os.environ["CODE_DIR"]  + f"/result/tawpipe-trace-{dist.get_rank()}.json")

    if dist.get_rank() == 0:
        t = np.mean(dts[2:])
        print(t)
        output_statistics("tawpipe", t, memory)
